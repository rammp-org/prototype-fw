...
- Motor control via the upstream [`espp::Mcp266`](https://github.com/esp-cpp/espp/tree/main/components/mcp266) component (espp >= 1.3.0) over **CANopen (CiA 301 / CiA 402)** — MIB owns the `espp::Twai` + `espp::CanopenClient` transport and drives the dual-channel (`M1` / `M2`) controller on top.
- **Position control** via CiA 402 profile position mode (`move_to_position`, `configure_position_loop`, `set_position_limits`) for both axes — validated on hardware.
...
The `app_main()` demo configures M1, runs a profile-position setpoint sequence, then a continuous ±10000 ping-pong interleaved with Ethernet status reporting. Set `-DMIB_SCAN_OD=1` at build time to dump the MCP's SDO object dictionary at startup (useful when mapping a device).

### Control notes / known limitations

These are properties of the MCP266 device / firmware, handled by the `espp::Mcp266` component:

- The MCP266's control-loop parameters are **not** standard CiA 402 objects. The MCP mirrors its packet-serial command set into the manufacturer region at object index `0x2000 + command number`; the component uses that to configure the position PID (commands 61–64) and read telemetry (24 / 82).
- The position loop's `MinPos`/`MaxPos` clamp defaults to `[0, 0]` (every target forced to zero) and its P gain can read back as zero; `configure_position_loop()` widens the clamp and (only if the stored P is zero) seeds a coarse fallback P gain each boot — the MCP reverts to EEPROM at power-up. Note the position-PID setter (cmds 61/62) uses field order `D, P, I` while the readback (63/64) uses `P, I, D`.
